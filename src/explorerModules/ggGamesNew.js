...
const fetchItemsByBaseUrl = (baseUrl) => {
  const items = [];
  const seen = {};
  let stop = false;
...
const getItems = () => {
  ...
  const baseUrls = [
    `https://gameguru.ru/games/platform_pc/release_${year}-${year + 1}/`,
    `https://gameguru.ru/games/platform_pc/release_${year}/`,
    'https://gameguru.ru/games/platform_pc/',
  ];

  const tryNext = () => {
    if (!baseUrls.length) return Promise.resolve([]);
    const baseUrl = baseUrls.shift();
    return fetchItemsByBaseUrl(baseUrl).catch(err => {
      if (err && err.statusCode === 404) {
        return tryNext();
      }
      throw err;
